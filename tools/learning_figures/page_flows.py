...
def entries(manifest: dict, root: Path = ROOT):
    ...
    for lesson, cfg in manifest.items():
        ...
        if not refs:
            continue
        if cfg.get('layout', 'lesson') != 'lesson':
            raise ContractError(f'PAGE-FLOW-INTEGRATION-001: {lesson}: page_figures currently require layout=lesson')
        ...
        for ref in refs:
            ...
            seen.add(ref['id'])
            if ref['slot'] not in SLOTS:
                raise ContractError(f'PAGE-FLOW-INTEGRATION-001: unsupported slot {ref["slot"]!r}')
...
def static_check(manifest: dict, root: Path = ROOT) -> list[str]:
    ...
    for required in (layout, renderer, slot_renderer, css):
    ...
    if layout.is_file():
        ...
        if "'/assets/css/learning-figure.css' | relative_url" not in text:
            problems.append('PAGE-FLOW-INTEGRATION-001: lesson layout must load learning-figure.css via relative_url')
        for slot in sorted({ref['slot'] for cfg in manifest.values() for ref in cfg.get('page_figures', [])}):
            token = f'{{% include learning-page-figure-slot.html slot="{slot}" %}}'
            if text.count(token) != 1:
                problems.append(f'PAGE-FLOW-INTEGRATION-001: layout requires exactly one {token}')
    try:
